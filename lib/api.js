...
function formatLoginError(login, fallback = '登录失败') {
  const status = get(login, 'status');
  const data = get(login, 'data', get(login, 'body.data'));
  const msg = get(login, 'message') || get(login, 'msg') || get(login, 'body.message') || get(login, 'body.msg');
  const errorCode = get(login, 'error_code', get(login, 'body.error_code'));

  const parts = [];
  if (msg) parts.push(msg);
  else if (typeof data === 'string' && data.trim()) parts.push(data.trim());
  else if (data && typeof data === 'object') parts.push(JSON.stringify(data));
  else if (fallback) parts.push(fallback);

  if (status !== undefined && status !== 1) parts.push(`status: ${status}`);
  if (errorCode !== undefined) parts.push(`error_code: ${errorCode}`);
  return parts.join(' · ');
}
...
export async function loginWithCode(mobile, code) {
  ...
  if (get(login, 'status') !== 1) {
    throw new Error(formatLoginError(login));
  }
...
export async function loginQrKey() {
  const r = await api('GET', '/login/qr/key', { timestamp: Date.now() });
  const data = get(r, 'data', get(r, 'body.data', get(r, 'body', r)));
  const qrUrl = data?.url || data?.qrurl || data?.qr_url || '';
  let key = '';
  if (qrUrl) {
    try {
      key = new URL(qrUrl).searchParams.get('qrcode') || '';
    } catch (_e) {
      key = '';
    }
  }
  key =
    ...
    get(r, 'data.key') ||
    get(r, 'data.unikey') ||
    get(r, 'body.data.qrcode') ||
    get(r, 'body.data.key') ||
    get(r, 'body.data.unikey') ||
    get(r, 'key') ||
    get(r, 'unikey');
  if (!key) {
    throw new Error(get(r, 'message') || get(r, 'msg') || get(r, 'body.message') || get(r, 'body.msg') || '获取二维码 key 失败');
  }
  return { raw: r, key, qrUrl };
}

export async function loginQrCreate(key) {
  const r = await api('GET', '/login/qr/create', { key, qrimg: true, timestamp: Date.now() });
  const data = get(r, 'data', get(r, 'body.data', get(r, 'body', r)));
  const qrText = data?.qrcode || data?.url || data?.qrurl || data?.qr_url || '';
  const base64 = data?.qrcode_img || data?.base64 || data?.qrimg || '';
  if (!qrText) {
    throw new Error(get(r, 'message') || get(r, 'msg') || get(r, 'body.message') || get(r, 'body.msg') || '生成二维码失败');
  }
  return { raw: r, qrText, base64 };
}

export async function loginQrCheck(key) {
  return api('GET', '/login/qr/check', { key, timestamp: Date.now() });
}
